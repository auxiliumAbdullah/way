    ...
    case 'pages':
      ...
      switch ($_GET['sub_view']) {
        ...
        case 'edit_page':
		  ...
		  $cats = $db->query("SELECT m.page_category, GROUP_CONCAT(c.category_name SEPARATOR ',') AS cats FROM pages AS m, pages_categories AS c WHERE FIND_IN_SET(c.category_id, m.page_category) != 0 AND m.page_id = ".$_GET['id']." GROUP BY m.page_category");
			$allcats = $cats->fetch_assoc();
			$ids = explode(",",$allcats['page_category']);
          ...
          $data = $get_data->fetch_assoc();
		  $data['article']['all_cats'] = $all_cats;
