<div class="post-comments">
  ...
  {else}
    ...
    {if $article['comments'] > 0}
      <div class="comments-filter">
        <div class="btn-group btn-group-sm js_comments-filter" data-value="post_comments">
          ...
          <div class="dropdown-menu dropdown-menu-left">
            <div class="dropdown-item pointer" data-value="post_comments" data-id="{$article['post_id']}">{__("Most Recent")}</div>
            <div class="dropdown-item pointer" data-value="post_comments_top" data-id="{$article['post_id']}">{__("Top Comments")}</div>
            <div class="dropdown-item pointer" data-value="post_comments_all" data-id="{$article['post_id']}">{__("All Comments")}</div>
          </div>
        </div>
      </div>
    {/if}
    ...
    <!-- post comment -->
    {include file='__feeds_comment.form.tpl' _handle='post' _id=$article['post_id']}
    <!-- post comment -->
    ...
    <!-- comments -->
    <ul class="js_comments {if $article['comments'] > 0}pt10{/if} {if $_live_comments}js_live-comments{/if}">
      {if $article['comments'] > 0}
    ...
    <!-- previous comments -->
    {if $article['comments'] >= $system['min_results']}
      <div class="ptb10 text-center js_see-more" data-get="post_comments" data-id="{$article['post_id']}" data-remove="true" data-target-stream=".js_comments">
        <span class="text-link">
          <i class="fa fa-comment"></i>
          {__("View previous comments")}
        </span>
