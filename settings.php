  ...
  switch ($_GET['view']) {
    ...
    case 'profile':
      ...
      switch ($_GET['sub_view']) {
		...
		$cats = $db->query("SELECT m.interest, GROUP_CONCAT(c.category_name SEPARATOR ',') AS cats FROM users AS m, pages_categories AS c WHERE FIND_IN_SET(c.category_id, m.interest) != 0 AND m.user_id = ".$user->_data['user_id']." GROUP BY m.interest");
			$allcats = $cats->fetch_assoc();
			$ids = explode(",",$allcats['interest']);
