...
class User
{
   ...
   * @return integer
   */
  public function post_article($publish_to, $page_id, $group_id, $event_id, $title, $text, $cover, $category_id, $tags, $type)
  {
    global $db, $system, $date;
    ...
    /* insert article */
    $db->query(sprintf("INSERT INTO posts_articles (post_id, cover, title, text, category_id, tags, type) VALUES (%s, %s, %s, %s, %s, %s, %s)", secure($post_id, 'int'), secure($cover), secure($title), secure($clean_text), secure($category_id), secure($tags), secure($type, 'int'))) or _error("SQL_ERROR_THROWEN");
    /* points balance */
   ...
   * @return void
   */
  public function edit_article($post_id, $title, $text, $cover, $category_id, $tags, $type)
  {
    global $db, $system, $date;
    /* check if blogs enabled */
    if (!$system['blogs_enabled']) {
    ...
    /* update the article */
    $db->query(sprintf("UPDATE posts_articles SET cover = %s, title = %s, text = %s, category_id = %s, tags = %s, type = %s WHERE post_id = %s", secure($cover), secure($title), secure($clean_text), secure($category_id), secure($tags), secure($type, 'int'), secure($post_id, 'int'))) or _error("SQL_ERROR_THROWEN");
  }
  ...
  public function settings($edit, $args)
  {
    ...
    switch ($edit) {
      ...
      case 'started':
        ...
        /* update user */
        $db->query(sprintf("UPDATE users SET user_country = %s, user_work_title = %s, user_work_place = %s, user_work_url = %s, user_current_city = %s, user_hometown = %s, user_edu_major = %s, user_edu_school = %s, user_edu_class = %s, user_firstname = %s, user_lastname = %s, user_name = %s, user_gender = %s WHERE user_id = %s", secure($args['country'], 'int'), secure($args['work_title']), secure($args['work_place']), secure($args['work_url']), secure($args['city']), secure($args['hometown']), secure($args['edu_major']), secure($args['edu_school']), secure($args['edu_class']), secure($args['user_firstname']), secure($args['user_lastname']), secure($args['user_name']), secure($args['user_gender']), secure($this->_data['user_id'], 'int'))) or _error("SQL_ERROR_THROWEN");
        break;
    }
  }
  ...
  public function sign_in_phone($phone, $code, $remember = false)
  {
    global $db, $system, $date;
