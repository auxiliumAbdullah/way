...
try {
  ...
  switch ($_GET['do']) {
    ...
    case 'add_category':
      ...
      /* insert */
      $db->query(sprintf("INSERT INTO pages_categories (category_icon,category_name, category_description, category_parent_id, category_order) VALUES (%s,%s, %s, %s, %s)", secure($system['system_uploads']. "/" .$_POST['cover']), secure($_POST['category_name']),  secure($_POST['category_description']), secure($_POST['category_parent_id'], 'int'), secure($_POST['category_order'], 'int'))) or _error("SQL_ERROR_THROWEN");
      /* return */
    ...
    case 'edit_category':
      ...
      /* update */
      $db->query(sprintf("UPDATE pages_categories SET category_icon = %s,category_name = %s, category_description = %s, category_parent_id = %s, category_order = %s WHERE category_id = %s",secure($system['system_uploads']. "/" .$_POST['cover']), secure($_POST['category_name']), secure($_POST['category_description']), secure($_POST['category_parent_id'], 'int'), secure($_POST['category_order'], 'int'), secure($_GET['id'], 'int'))) or _error("SQL_ERROR_THROWEN");
      /* return */
