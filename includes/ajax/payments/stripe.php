...
try {
  ...
  switch ($_POST['handle']) {
	...
	global $db, $system, $date;
      ...
      if ($charge) {
		  $db->query(sprintf("INSERT INTO wallet_transactions (user_id, node_type, node_id, amount, type, date) VALUES (%s, %s, %s, %s, %s, %s)", secure($user->_data['user_id'], 'int'), secure("product"), secure(0, 'int'), secure($_POST['price']), secure("out"), secure($date))) or _error("SQL_ERROR_THROWEN");
        // update user package
        //$user->update_user_package($package['package_id'], $package['name'], $package['price'], $package['verification_badge_enabled']);
      }

      // return
      return_json(array('callback' => 'window.location.href = "' . $system['system_url'] . '/wallet";'));
      break;  
